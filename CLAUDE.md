...
3. **Statistics screen** — _(partially implemented)_ `/statistics` shows a line graph of each house's point total over the last 4 days (one line per house, built with [layerchart](https://next.layerchart.com)), reconstructed from `point_transactions`, and a bar graph of each active professor's point activity over that same 4-day window (points gifted vs. subtracted, per professor). See `src/routes/statistics/CLAUDE.md` for implementation details. Not yet built: a period selector, and an all-time "which professor has awarded the most points overall" view (the current professor chart is windowed to the same last-4-days period as the house chart, not all-time).
